/** @fileoverview Highlights widget dor Janelas Virtuais/Tela Social
	@version 3.0
 ...
 * ***** END LICENSE BLOCK ***** */
 ...
 /**
	Objeto com as funções para o widget highlights
	@class
	@name highlights
 */
var highlights = {
	...
	@type String
	*/
	feedURL : "http://query.yahooapis.com/v1/public/yql?q=select * from rss where url=\'"+ICMC_DESTAQUES+"\'&format=json&callback=",
	/** 
	...
	*/
	...
	/**
		...
		O método {@link highlights.start} chama está função
		@private
		@function
	*/
	loadFeed : function() {
		$.getJSON(highlights.feedURL,function(e){
			console.log(e.query.results);
			highlights.createQueue(e.query.results);
			});
		/*this.feed.load(function(e) {
			highlights.createQueue(e)
		});*/
	},
	/**
		Cria a fila de noticias
		O método {@link highlights.loadFeed} chama está função
	...
	*/
	createQueue : function(e) {
		...
		highlights.counter = highlights.queue.length * 30;
	...
	/**
		...
		Após processar os itens, o método {@link highlights.createQueue} chama está função
		@private
		@function
	*/
	render : function() {
		if(highlights.queue.length > 0) {
			
			var obj = highlights.queue.pop();
			...
			var qr = $('<img>').attr('src',highlights.urlQR + link).addClass('gv200').addClass('gh200');
			var shortenlink = $('<div>').addClass('link').html(link);
			fr.append(qr).append(shortenlink);
			item.append(fr);
			item.appendTo(highlights.container);;
			setTimeout(function() {
				highlights.render();
			}, 100);
		} else {
	...
	*/
	swap : function() {
		if(highlights.counter > 0) {
			$($(highlights.container).find('.item')[0]).fadeOut(1000, function() {
				$(this).remove().appendTo(highlights.container).show().css('z-index', highlights.z--);
				highlights.counter--;
				setTimeout(function() {
					highlights.swap();
				}, 7000);
